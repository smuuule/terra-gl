...
std::unordered_map<std::string, std::vector<time_event_durations_t>>
    time_recordings;
...
void popTimer() {
  ...
  if (event_stack.size() > 1) {
    event_stack[event_stack.size() - 2].children.push_back(
        std::move(event_stack[event_stack.size() - 1]));
...
Scope::Scope(const std::string &name) { pushTimer(name); }

Scope::~Scope() { popTimer(); }
...
void draw_time_column(duration_t d) {
  ...
  ImVec4 c(1, 1, 1, 1);
  c.z = std::min(1.f, std::max(0.f, 1.f - (s - settings.yellow_start) /
                                              (settings.yellow_end -
                                               settings.yellow_start)));
  c.y = std::min(1.f, std::max(0.f, 1.f - (s - settings.orange_start) /
                                              (settings.orange_end -
                                               settings.orange_start)));
...
void draw_events(time_event_t &e, const std::string &path) {
  ...
  avg = avg * settings.running_avg_mult +
        e.duration * (1 - settings.running_avg_mult);
  ...
  if (open) {
    std::sort(e.children.begin(), e.children.end(),
              [](const time_event_t &a, const time_event_t &b) -> bool {
...
std::string printf_events() {
  ...
  const auto count_name_size = [&](const time_event_t &e) -> void {
    auto count_impl = [&](const time_event_t &e, size_t depth,
                          auto &count_ref) mutable -> void {
  ...
  const auto stringify = [&](const time_event_t &e) -> std::string {
    auto stringify_impl = [&](const time_event_t &e, std::string depth,
                              auto &stringify_ref) mutable -> std::string {
      std::string s;
...
void record_events() {
  const auto record_rec = [&](const time_event_t &e) {
    auto record_rec_impl = [&](const time_event_t &e,
                               const std::string &event_path,
                               auto &record_rec_ref) mutable -> void {
...
void drawEventsWindow() {
  ...
  ImGui::Begin("Performance Timings");
  {
    ...
    float settingsButtonWidth = ImGui::CalcTextSize("Settings").x +
    ...
    if (ImGui::BeginPopupModal("Settings##Perf Settings Popup")) {
      ...
      ImGui::SliderFloat("Running Average Multiplier", &avg, 0.0001f, 1.f,
                         "%.5f", ImGuiSliderFlags_Logarithmic);
      settings.running_avg_mult = 1.f - avg;
      ImGui::SliderFloat("White to Yellow Value (ms)", &settings.yellow_start,
                         0.01f, 33.3f, "%.2f");
      ImGui::SliderFloat("100% Yellow Value (ms)", &settings.yellow_end, 0.01f,
                         33.3f, "%.2f");
      if (settings.yellow_end <= settings.yellow_start + 1e-7f) {
        settings.yellow_end = settings.yellow_start + 0.1;
      }
      ImGui::SliderFloat("Yellow to Red Value (ms)", &settings.orange_start,
                         0.01f, 33.3f, "%.2f");
      if (settings.orange_start <= settings.yellow_end + 1e-7f) {
        settings.orange_start = settings.yellow_end + 0.1;
      }
      ImGui::SliderFloat("100% Red Value (ms)", &settings.orange_end, 0.01f,
                         60.f, "%.2f");
...
void free_query(uint32_t q) { query_pool.push_back(q); }
...
#define checkCudaErr(expr)                                                     \
  do {                                                                         \
    ...
    if (_status != 0) {                                                        \
      cudaDeviceReset();                                                       \
      LOG_FATAL("CUDA error in {}({}): {}", __FILE__, __LINE__,                \
...
void flushCUDA() {
  if (last_recorded_event != nullptr) {
    // This is most probably unnecessary since deviceSynch is supposed to synch
    // *everything*
...
void free_event(cudaEvent_t evt) { event_pool.push_back(evt); }
